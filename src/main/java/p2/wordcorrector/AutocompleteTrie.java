...
public class AutocompleteTrie extends HashTrieMap<Character, AlphabeticString, Integer> {
    ...
    public String autocomplete(String key) {
        ...
        for (Character item : key.toCharArray()) {
            if (current.pointers.find(item) == null) { //couldnt find item (null)
                return null;
            }
            else {
        ...
        while (current.pointers.size() == 1) {
            ...
            result.append(current.pointers.iterator().next()); //updated for use of chaining hash
            current = current.pointers.iterator().next().value; //.value gets the value(hashtrienode) of the iterator<K, hashtrienode>
        }
