 ...
 */
//the type of type of the chain should be generic, so we can use any data structure that implements the dictionary interface to chain the keys in each bucket (index)
public class ChainingHashTable<K, V> extends DeletelessDictionary<K, V> {
    ...
    int capacity; // the capacity of the hashtable, used for hashing, needs to be a prime number, and will be updated when rehashing
    double loadFactor; // the load factor of the hashtable
    private Dictionary<K, V>[] bucket; //an array where each element is a dictionary that will store the keys that hash to that index
    int primeIndex=0; //the index of the prime number in the PRIME_SIZES array that we are currently using
    ...
    public ChainingHashTable(Supplier<Dictionary<K, V>> newChain) {
        ...
        //supplier can be any data structure that implements the dictionary interface, such as AVLTree or MoveToFrontList
    ...
    public V insert(K key, V value) {
        ...
        //call find with the key on the data structure at the index i (bucket[i]) find if the key is already in the dictionary, if it is, update the value
        ...
        V oldValue = bucket[i].insert(key, value); //insert the key value pair into the dictionary at the index i, calling insert returns the old value
        if (oldValue == null) {
            size++; //if the old value is null, that means the key was not in the dictionary, so we have added a new key value pair
        }
     ...
     */
        ...
        public boolean hasNext() {
            ...
            //if the current iterator is null, return false, we are at the last bucket and it is empty
            if (bucketIterator == null) {
                return false; 
            }

            //else the next bucket has an object, check if that object has a next element
            //should get the next bucket that has an object, if no next bucket exists then it will end on the last bucket
    ...
    return ArrayIterator; //return the iterator we created
